...
/*--------------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  function _setLevelStyleChanges(colorText, colorBG) {
    /*--------------------------- partial function for set level of the game (it´s also about change styles)*/

    //style -> color of H1, H3 and seconds
    dispatch({ type: "SET_COLOR", payload: colorText });
    _stylingAfterLevel(
      colorBG,
    ); /*----------------------------------------------------partial f. with style changes after select level*/
  }
  ...
  function shuffle() {
    /*-------------------------------------------------------------function for shuffling (ONLY) in harder and the hardest version of game*/

    if (state.level === "harder" || state.level === "hardest") {
      /*this method is for the hardest level .. it´s maybe slower because of rerendering */
      //get HTMLcollection
      let x =
        document.getElementsByClassName(
          "div_on_click",
        ); /*--------------------collection of divs above image*/

      //convert collection to array
      let arr = Array.from(x);
      _shuffleArray(
        arr,
      ); /*--------------------------------------------------------partial f. to random shuffle of array, f. included from _inc_functions.js */
  ...
  return (
    <>
      <div className="welcome">
        ...
        <h3 style={{ color: state.color }}>
          Vitajte v hre pexeso, pre začatie hry zvoľte náročnosť nižšie{" "}
        </h3>
