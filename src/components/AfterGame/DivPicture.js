...
export const DivPicture = (props) => {
  ...
  function checkEnd() {
    ...
    if (!document.getElementById("row").firstElementChild) {
      /*--------------------if all images on page are removed */
      ...
      document
        ...
        .firstElementChild.classList.add(
          "div_center",
        ); /*---------------start ---animation of gratulation text */
      let timeArr =
        endTime.split(
          ":",
      ...
      document
        ...
        .classList.add(
          "h1End",
        ); /*---------end ---animation of gratulation text */
    }
  }
  ...
  function _deleteImg(el, checkEndCallBack) {
    /*-----------------------------------------partial f. to remove the same showed images*/
  ...
  function _animate(element, deleteImgCallBack, checkEndCallBack) {
    /*-------------------animation to rotate -> then delete div>img*/
  ...
  function animateAndDelete(first, second, settingAfterComparisonCallBack) {
    /*------------------------..-----------------matched img´s animate->delete*/
    ...
    if (second) {
      ...
      const selectedImgAgain =
        document.getElementsByClassName("selected_Div_img");
  ...
  function hideUnMatched(first, second, settingAfterComparisonCallBack) {
    /*---------------------------------------------if shown images do not match -> hide them back  */
    _myToggle(first, "selected_Div_img", "mask");

    if (second) {
      _myToggle(second, "selected_Div_img", "mask");
    } else {
      const selectedImgCol =
        document.getElementsByClassName("selected_Div_img");
  ...
  function mainFn(element) {
    /*---------------------------------------------------------the most main function to manage pexeso-code */
    if (element.classList.contains("mask")) {
      /*------------------------------------------if on image is joker´s image */

      var imgElm = element.firstElementChild;
      showImg(
        element,
      ); /*--------------------------------------------------------------remove joker image and show img under */

      if (props.stticSource === "") {
        /*----------------------------------------------------if no image is shown, get attribute from clicked*/

        props.setStticSource(imgElm.getAttribute("src"));
      } else {
        /*-------------------------------------------------------------------------compare sources attribute of showed and clicked */
        document.body.style.pointerEvents =
          "none"; /*--------------------------------prevent to show third image */
        ...
        do {
          ...
          if (iterationCount > maxIterations) {
            /*------------------------------------prevent freezing browser and game */
            console.warn("it is a never ending loop ");
            break;
          }
        } while (selectedImgCol.length !== 2);

        if (props.stticSource === imgElm.getAttribute("src")) {
          /*------------------------if matched --> remove images */
          ...
          /*---------------------------------------------------------------------if NOT the same src-path --> hide images below joker img */
  ...
  return (
    <div
      onClick={(e) => {
        mainFn(e.target.parentNode);
      }}
      className="mask div_on_click"
    >
      <img
        src={"pictures/pexeso/" + props.pictureName + ".jpg"}
        alt="Smiley face"
      />
    </div>
  );
