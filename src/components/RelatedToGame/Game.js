 ...
 /*--------------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      try {
        // decrypting of data
        const decryptedSettings = simpleCrypto.decrypt(
          decodeURIComponent(settingsData),
        );
        ...
        document
          ...
          .setAttribute("style", "background-color: " + state.colorBG);
      } catch (error) {
  ...
  return (
    <>
      <div className="welcome">
        <a href="/settings" className="end-game-btn">
          {" "}
          {state.linkName}{" "}
        </a>

        <h3 style={{ color: state.colorText }}>
          {" "}
